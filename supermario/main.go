...
import (
	...
	audioengine "apple/supermario/audio-engine"
...
func main() {
	...
	rl.InitWindow(screenWidth, screenHeight, "supermario")
	var Health int = 100
	var Score int = 0
	grounds := []Ground{}
	bullets := []Bullet{}
	...
	music := rl.LoadSound("supermario/assets/bgm.mp3")
	rl.PlaySound(music)
	...
	Character := rl.LoadImage("supermario/assets/mario.png")
	LeftCharacter := rl.LoadImage("supermario/assets/leftmario.png")
	Enemies := []Enemy{}
	RightGoomba := rl.LoadImage("supermario/assets/leftgoomba.png")
	LeftGoomba := rl.LoadImage("supermario/assets/rightgoomba.png")
	...
	for !rl.WindowShouldClose() {
		...
		for _, current_ground := range grounds {
			...
			if rl.CheckCollisionRecs(rl.NewRectangle(float32(x_coords), float32(y_coords), float32(50), float32(50)), rl.NewRectangle(float32(current_ground.posX), float32(current_ground.posY), float32(current_ground.width), float32(current_ground.height))) {
				...
				if rl.IsKeyDown(rl.KeyW) || rl.IsKeyDown(rl.KeySpace) {
					rl.PlaySound(rl.LoadSound("supermario/assets/jump.mp3"))
					y_coords -= 300
				}
			}
		...
		if rl.IsKeyDown(rl.KeyJ) {
			rl.PlaySound(rl.LoadSound("supermario/assets/shoot.mp3"))
			if isRight {
				if should_shoot {
		...
		if Health <= 0 {
			...
			rl.DrawText("U, Dead~ man~", 150, 150, 80, rl.Red)
			rl.DrawText("Final Score : "+strconv.Itoa(Score), 250, 250, 30, rl.Red)
			if flag {
				rl.UnloadSound(music)
				audioengine.DieSound()
				flag = false
			}
		}
		rl.EndDrawing()
